...
export async function getConsumptionStatisticByDateAnTgId(
  ...
  if (foods.length === 0) {
    await ctx.reply(
      `You don't have any consumption records ${
        isDateRange ? "in this date range" : "in this day"
      }`
    );
  ...
  if (checkForList) {
    let productInfo = `List of consumed products (for ${dateLabel}):\n`;
    foods.forEach((food) => {
  ...
  const productInfo = `
Date${isDateRange ? " range" : ""}: ${dateLabel}
